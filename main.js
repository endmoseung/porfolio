...
const headboxContentButtonActive = document.querySelector(
  ".headbox__content--button.active"
);
const contactMeButton = document.querySelector(".contactme");
const headboxContentButton = document.querySelector(
  ".headbox__content--button"
);
...
document.addEventListener("scroll", () => {
  if (window.scrollY > homeheight) {
    arrowBtn.classList.add("active"); //시간이 일정시간 지났을때 함수호출 여기서는 5000ms 즉 5초뒤에 arrowbtn이 사라지도록 함
  } else {
...
projectPosition.addEventListener("click", (event) => {
  const filter =
    event.target.dataset.filter || event.target.parentNode.dataset.filter; //or 오른쪽에 경우는 버튼안에 숫자에 관한 경운데 이 숫자의 부모요소의 Dataset은 or 왼쪽의 data와 같다
  if (filter == null) {
    return;
  }
...
window.addEventListener("wheel", () => {
  ...
  if (window.scrollY === 0) {
    ...
    //이떄까지 내린 scrolly의 값과 현재 윈도우height의 합은 총 페이지의 하잇과 같다
